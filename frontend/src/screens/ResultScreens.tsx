...
export function AnalyzeCodeResultsScreen({ report, onRestart, onRepair }: { report: RunReport; onRestart: () => void; onRepair?: () => void }) {
  ...
  const noRepairNeeded = !repaired && report.baseline_scan_status === 'completed' && report.baseline_findings.length === 0
  return <ResultLayout testId="analyze-results">
    <div data-testid="upload-result-state" data-state={repaired ? 'post-repair' : noRepairNeeded ? 'no-repair-needed' : 'initial-scan'}>
...
export function GenerateEvaluateResultsScreen({ report, onRestart }: { report: RunReport; onRestart: () => void }) {
  const best = report.strategy_results.find(result => result.strategy_id === report.best_overall) ?? report.strategy_results[0]
  const noRepairNeeded = !best && report.baseline_scan_status === 'completed' && report.baseline_findings.length === 0
  return <ResultLayout testId="generate-results">
    <ResultHeader eyebrow="Exploratory" title="Generate & Evaluate Results" description={noRepairNeeded ? 'Review the generated code, scanner evidence, and smoke-check status from this run.' : 'Compare the generated code with the AI-assisted repair, scanner evidence, and smoke-check status from this run.'} report={report} />
    <Note title="How you got here"><p>{noRepairNeeded ? 'Generate & Scan completed the baseline evidence. No repair stage was needed.' : 'Generate & Scan collected the baseline evidence. After Repair & Compare, this page shows only persisted before-and-after backend results.'}</p></Note>
    <section className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Original findings" value={report.baseline_scan_status === 'completed' ? String(report.baseline_findings.length) : 'Unavailable'} tone="orange" /><Metric label="After repair" value={noRepairNeeded ? 'Not run' : best?.repaired_scan_status === 'completed' ? String(best.repaired_findings.length) : 'Unavailable'} tone="green" /><Metric label="Smoke check" value={testCounts(report.baseline_tests, 'Smoke check')} /><Metric label="Classification" value="Exploratory" tone="blue" /></section>
    ...
    {noRepairNeeded ? <><section className="mt-3 rounded-xl border border-emerald-500/25 bg-emerald-500/[0.07] p-4"><h2 className="text-sm font-semibold text-emerald-300">No security findings detected</h2><p className="mt-2 text-xs leading-5 text-[#B8BEC7]">Repair was not run because there were no findings to repair.</p></section><section className="mt-3 grid gap-3 lg:grid-cols-2"><CodePanel title="Generated code" subtitle="Exact code returned by the live model" code={report.baseline_source} /><FindingsPanel title="Generated-code findings" subtitle="Bandit and Semgrep baseline evidence" findings={report.baseline_findings} status={report.baseline_scan_status} /></section></> : <><section data-testid="custom-before-after" className="mt-3 grid gap-3 lg:grid-cols-2"><CodePanel title="Before repair" subtitle="Exact code returned by the live model" code={report.baseline_source} /><CodePanel title="After repair" subtitle="Exact repaired code returned by the repair step" code={best?.repaired_code ?? ''} /></section><section className="mt-3 grid gap-3 lg:grid-cols-2"><FindingsPanel title="Generated-code findings" subtitle="Bandit and Semgrep baseline evidence" findings={report.baseline_findings} status={report.baseline_scan_status} /><FindingsPanel title="Repaired-code findings" subtitle="Bandit and Semgrep after repair" findings={best?.repaired_findings ?? []} status={best?.repaired_scan_status ?? 'unavailable'} /></section><StrategyTable report={report} mode="custom" /></>}
    <FinalExplanation title={noRepairNeeded ? 'What this result means' : 'What the comparison means'} report={report} warning="Fewer configured SAST findings are useful evidence, not proof that the program is fully secure." onRestart={onRestart} />
  </ResultLayout>
}
