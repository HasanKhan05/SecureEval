...
export function LiveAnalysisScreen({ mode, progress, scans, error, terminalMessage, onDone, onBack, onCancel }: { mode: 'benchmark' | 'upload'; progress: RunProgress | null; scans: ScanCategoryId[]; error: string | null; terminalMessage: string | null; onDone: () => void; onBack: () => void; onCancel: () => void }) {
  ...
  return (
    <Shell>
      <Header eyebrow={staticOnly ? UPLOAD_EVIDENCE_COPY.eyebrow : BENCHMARK_EVIDENCE_COPY.eyebrow} title="Security Analysis" description={staticOnly ? 'SecureEval sends uploaded code only to the local backend for syntax validation and static analysis. It is never executed, and static analysis is not a security guarantee.' : 'SecureEval is running the generated program through real local functional tests, Bandit, and Semgrep. Results are evidence from this sample run, not a security guarantee.'} />
      ...
      <div className="rounded-xl border border-black/[0.08] bg-white p-5 shadow-sm sm:p-7">
        ...
        {ready && <div className="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 p-4"><div className="font-display font-black uppercase text-emerald-900">Baseline evidence saved</div><p className="mt-1 text-xs text-emerald-800">{staticOnly ? `${UPLOAD_EVIDENCE_COPY.functionalTestsUnavailable} Syntax and scanner evidence are saved; choose repair strategies to continue.` : 'Functional test and scanner evidence is saved. Choose repair strategies to continue.'}</p></div>}
...
export function LiveComparisonScreen({ mode, progress, report, strategies, error, terminalMessage, onDone, onCancel, onBack }: { mode: 'benchmark' | 'upload'; progress: RunProgress | null; report: RunReport | null; strategies: StrategyId[]; error: string | null; terminalMessage: string | null; onDone: () => void; onCancel: () => void; onBack: () => void }) {
  ...
  return (
    <Shell>
      <Header eyebrow={staticOnly ? UPLOAD_EVIDENCE_COPY.eyebrow : 'Live repair evaluation'} title="Strategy Comparison" description={staticOnly ? 'Repairs are evaluated with syntax validation and static scanners only. Uploaded code and repaired candidates are never executed; static analysis is not a security guarantee.' : 'Each selected strategy is repaired, functionally tested, rescanned, and scored by deterministic application logic.'} />
      {error && <ErrorPanel message={error} />}
      <div className="rounded-xl border border-black/[0.08] bg-white p-5 shadow-sm sm:p-7">
        <ProgressBar progress={progress} staticOnly={staticOnly} />
    ...
    <Shell>
      ...
      <section className="mb-7 rounded-xl border border-black/[0.08] bg-white p-5 shadow-sm sm:p-7">
        ...
        <div className="mt-4 space-y-3">
          ...
          {staticOnly && <div><h3 className="mt-5 font-mono text-[10px] uppercase tracking-widest text-slate-400">Uploaded source code</h3><pre className="mt-2 max-h-80 overflow-auto rounded-lg bg-slate-950 p-4 text-xs leading-relaxed text-slate-100">{report.baseline_source || 'Source unavailable.'}</pre></div>}
        </div>
      </section>
      ...
      <section className="rounded-xl border border-[#1B3A6B]/20 bg-[#1B3A6B]/5 p-5 sm:p-7">
        <div className="font-mono text-[9px] uppercase tracking-widest text-[#1B3A6B]">{staticOnly ? 'Persisted static-only result' : 'Persisted result'} · {report.explanation_source}</div>
