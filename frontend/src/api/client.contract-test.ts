...
import type { Run, RunProgress, RunReport } from "../contracts/api-v1";
...
const progress: Promise<RunProgress> = client.getProgress("run_opaque");
const configured: Promise<Run> = client.configureStrategies("run_opaque", [
  "scanner_feedback_v1",
]);
const report: Promise<RunReport> = client.getReport("run_opaque");

void [created, read, started, cancelled, progress, configured, report];
